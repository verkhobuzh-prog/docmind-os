# Архітектурний паспорт проекту Doc-Hub

| Поле | Значення |
|------|----------|
| **Назва** | Doc-Hub (DocumentOS) |
| **Тип** | Enterprise AI Document SaaS |
| **Версія архітектури** | 1.0 (Phase 1 MVP) |
| **Дата** | Травень 2026 |
| **Статус** | ✅ Phase 1 завершено (Backend API) |
| **Репозиторій** | https://github.com/verkhobuzh-prog/doc-hub |
...
Doc-Hub — масштабована AI-платформа для роботи з корпоративними документами: завантаження, індексація, семантичний пошук і діалог (RAG) з обов'язковим цитуванням джерел.
...
```mermaid
C4Context
    title Doc-Hub — System Context

    Person(user, "Користувач")
    Person(admin, "Адмін організації")
    System(dochub, "Doc-Hub")
    System_Ext(supabase, "Supabase")
    System_Ext(openai, "OpenAI API")

    Rel(user, dochub, "Upload, Chat, Search")
    Rel(admin, dochub, "RBAC, Billing")
    Rel(dochub, supabase, "Auth, DB, Storage")
    Rel(dochub, openai, "Embeddings, LLM")
```
...
*Документ є єдиним джерелом архітектурної правди (architecture single source of truth) для Doc-Hub на момент завершення Phase 1 MVP.*
